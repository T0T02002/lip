...
(* Converte un valore numerico (loc) in una stringa che rappresenta il numero *)
let string_of_val = function
  | n -> string_of_int n

(* Converte l'espressione in una stringa leggibile *)
...
(* Converte un comando in una stringa leggibile *)  
...
(* Converte una dichiarazione in una stringa leggibile*)
let string_of_decl = function
  | IntVar(x) -> "int " ^ x
  | Fun(f,x,c,e) -> "fun " ^ f ^ "(" ^ x ^ ") {" ^ string_of_cmd c ^ "return " ^ string_of_expr e ^ "}"

(* Trasforma una variabile o una funzione in una stringa, mostrando il suo contenuto nell'ultimo env *)
...
(* Converte una lista  di variabili o di funzioni dichiarate in stringhe leggibili *)  
...
(* Converte la memoria (una coppia (mem,loc)) in una stringa leggibile e ne mostra i valori associati all'indice in memoria *)                
let string_of_mem1 (mem,loc) index =
  assert (index<loc);
  string_of_val (mem loc) ^ "/" ^ string_of_int index

(* Genera una lista di numeri nell'intervallo a,b. Utilizzata per scorrere tra gli indici di memoria *)
let rec range a b = if b<a then [] else a::(range (a+1) b);;

(* Scorre una lista di coppie (mem,loc) e la rende una stringa leggibile *)
let string_of_mem (m,l) =
  List.fold_left (fun str i -> str ^ (try string_of_mem1 (m,l) i ^ "," with _ -> "")) "" (range 0 (l - 1))

(* Estrae le posizioni di memoria loc associate a variabili nell'ambiente *)
...
(* Rappresenta lo stato del programma in una stringa, includendo env, mem e ultima posizione di memoria allocata *)    
let string_of_state st dom =
  "[" ^ string_of_env st dom ^ "], " ^
  "[" ^ string_of_mem (getmem st,getloc st) ^ "]" ^ ", " ^
  string_of_int (getloc st)

(* Calcola l'unione di due lise evitando duplicati, combina variabili provenienti da espressioni o comandi *)
let rec union l1 l2 = match l1 with
    [] -> l2
  | x::l1' -> (if List.mem x l2 then [] else [x]) @ union l1' l2

(* Restituisce una lista di variabili expr *)
...
(* Restituisce una lista di variabili cmd *)
